...
from campaign.models import Order_notifications
...
def save_customer_delivery_info(request):
    ...
    try:
        ...
        var_customer_info = Customer_delivery_information(Customer=myuser,
                                                          ...
                                                          Town_City=customer_Town_City, District=customer_District,
...
def order_save_with_all_info(request):
    ...
    for cam_name in lst_cam_sts:
        ...
        get_Notification_text = f'A new order placed (id {new_order_id}) '

        order_base_notifications = Order_notifications(Notification_order=var_Order_Table, Notification_text = get_Notification_text)
        order_base_notifications.save()
    ...
    print(site.domain)
    var_dmain = site.domain
    
    success_var = 'http://'+ str(var_dmain) + '/payment-successful'
    fail_var = 'http://'+ str(var_dmain) + '/payment-failed'
    cancel_var = 'http://'+ str(var_dmain) + '/payment-cancelled'
    ...
    if order_delivery_option == 'Pay Online With SSLCommerz(Credit/Debit Card/MobileBanking/NetBanking/bKash)':
        ...
        post_body['success_url'] = success_var
        post_body['fail_url'] = fail_var
        post_body['cancel_url'] = cancel_var
        ...
        return redirect('Order-placed-successfully')
