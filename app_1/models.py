...
class Brand(models.Model):
    ...
    Brand_logo = models.ImageField(upload_to='Brand_logo/', blank=True, null=True, validators=[FileExtensionValidator(allowed_extensions=['jpg', 'png', 'jpeg', 'webp'])],
                                    help_text="Choose Only .jpg, .jpeg, .png, .webp files PLease..")
...
class Products(models.Model):
    ...
    who_add_this = models.CharField(max_length=255, blank=True, null=True, default='')
...
class Staff_Access(models.Model):
    ...
    product_upload_qty = models.IntegerField(blank=True, null=True, default=0)

    def __str__(self):
        who_add_this = self.Username + ',' + str(self.id)
        print(who_add_this)
        qty_prd = Products.objects.filter(who_add_this=who_add_this).count()
        self.product_upload_qty = qty_prd
        self.save()
