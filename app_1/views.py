...
def dashbord_add_new_products(request):
    ...
    if staff_admin or staff_shop_manager or staff_upload_team:
        ...
        if staff_admin:
            ...
            role_staff = staff_shop_manager
        elif staff_customer_support:
            role_staff = staff_customer_support
        elif staff_upload_team:
            role_staff = staff_upload_team

        get_staff = Staff_Access.objects.get(Username=role_staff)

        who_add_this = get_staff.Username + ',' + str(get_staff.id)
        ...
        if request.method == 'POST':
            ...
            if form_product.is_valid():
                ...
                other_value.who_add_this = who_add_this
                other_value.save()
...
def deshboard_product_find_by_search(request):
    ...
    if staff_admin or staff_shop_manager or staff_upload_team or staff_upload_team:
        ...
        Subcategory_1_filter_nam = Subcategory_1.objects.filter(Subcategory_1=get_find_by_search_product)

        all_product_show = Products.objects.filter(Q(Product_Name__icontains = get_find_by_search_product) | Q(Category__Category_Name__icontains = get_find_by_search_product) | Q(Subcategory_1__in = Subcategory_1_filter_nam))
        all_product_qunt = Products.objects.all().count()
